...
export async function GET(
    ...
    console.log("=== SoundCharts Artist Identifiers Request ===");
    ...
    const apiUrl = `https://customer.api.soundcharts.com/api/v2/artist/${encodeURIComponent(uuid)}/identifiers?offset=0&limit=100&onlyDefault=false`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            let errorMessage = `Soundcharts identifiers fetch failed: ${response.status}`;
            try {
        ...
        console.log("✅ Successfully fetched identifiers, count:", data?.items?.length || 0);
        ...
        return NextResponse.json(
            {
                error: error.message || "Failed to fetch identifiers from SoundCharts",
