...
export default function ExplorePage() {
  ...
  useEffect(() => {
  // 🔁 when search clear from search bar
  if (searchQuery.trim().length === 0) {
    const fetchDefaultExploreData = async () => {
      try {
        const [artistsRes, reviewsRes, genresRes] = await Promise.all([
          fetch("/api/explore/artists"),
          fetch("/api/explore/reviews"),
          fetch("/api/explore/genres"),
        ]);
        ...
        console.error("Failed to reload explore data", err);
      }
    };

    fetchDefaultExploreData();
    return;
  }
  ...
  const fetchSearchResults = async () => {
    try {
      const [artistsRes, reviewsRes, genresRes] = await Promise.all([
        fetch(`/api/explore/artists?search=${searchQuery}`),
        fetch(`/api/explore/reviews?search=${searchQuery}`),
        fetch(`/api/explore/genres?search=${searchQuery}`),
      ]);
