...
export default function AllChatPage() {
  ...
  const { onlineUsers, onlineUserDetails, onlineCount, typingUsers, broadcastTyping } = useCommunityPresence(communityId);
        ...
        (payload) => {
          setMessages(prev => prev.map(msg =>
            msg.forum_post_id === payload.new.forum_post_id
              ? { ...msg, is_pinned: payload.new.is_pinned, reactions: payload.new.reactions || {} }
              : msg
          ));
            ...
            setThreadReplies(prev => ({
              ...
              [parentId]: (prev[parentId] || []).map(reply =>
                reply.forum_post_id === payload.new.forum_post_id
                  ? { ...reply, reactions: payload.new.reactions || {} }
                  : reply
              )
            }));
          }
    ...
    try {
      ...
      const messagesWithUsers = await Promise.all(
        (data || []).map(async (msg) => {
          ...
          return { ...msg, user: userData, replyCount: count || 0, reactions: msg.reactions || {} };
        })
      );
  ...
  const sendMessage = async () => {
    if ((!inputMessage.trim() && selectedFiles.length === 0) || !forumId || !user) return;
      ...
      if (!error && data) {
        const repliesWithUsers = await Promise.all(
          data.map(async (msg) => {
            const userRes = await fetch(`/api/users/${msg.user_id}`);
            const { user: userData } = userRes.ok ? await userRes.json() : { user: null };
            return { ...msg, user: userData, reactions: msg.reactions || {} };
          })
        );
        
        setThreadReplies(prev => ({ ...prev, [postId]: repliesWithUsers }));
      }
                      ...
                      {Array.from(onlineUsers).map((userId) => {
                        ...
                        return (
                          <div key={userId} className="flex items-center gap-2 p-2 hover:bg-[#2d2838] rounded transition">
                            ...
                            <span className="text-gray-300 text-sm">{userInfo?.username || 'User'}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  {messages.length > 0 && (
                    <span className="text-[#FA6400]">{messages.length} new messages</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          {filteredMessages.map((msg, idx) => {
            const showDate = idx === 0 || 
              new Date(filteredMessages[idx - 1].created_at).toDateString() !== new Date(msg.created_at).toDateString();
            const prevUser = idx > 0 ? filteredMessages[idx - 1].user_id : null;
            const showAvatar = msg.user_id !== prevUser;
            const replies = threadReplies[msg.forum_post_id] || [];
                  ...
                  <div className="flex-1 min-w-0">
                    {showAvatar && (
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {msg.user_id !== prevUser && (
                          <span className="text-gray-500 text-xs">
                            last active {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </span>
                        )}
                      </div>
                    )}
                    ...
                    <div className="group">
                      ...
                      {msg.parent_post_id && (
                        <div className="text-xs text-gray-400 mb-1 flex items-center gap-1">
                          ...
                          Reply to {messages.find(m => m.forum_post_id === msg.parent_post_id)?.user?.username || 'message'}
                        </div>
                      )}
                      ...
                      {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {Object.entries(msg.reactions).map(([emoji, users]) => (
                            <button
                              key={emoji}
                              onClick={() => addReaction(msg.forum_post_id, emoji)}
                      ...
                      <div className="opacity-0 group-hover:opacity-100 transition flex gap-3 mt-2">
                        ...
                        <div className="relative">
                          <button
                            onClick={() => setShowReactionPicker(showReactionPicker === msg.forum_post_id ? null : msg.forum_post_id)}
                            className="text-sm text-gray-400 hover:text-white flex items-center gap-1.5 px-2 py-1 rounded hover:bg-[#2d2838]"
                          >
                            😀 React
                          </button>
                          {showReactionPicker === msg.forum_post_id && (
                            <div className="absolute bottom-full left-0 mb-2 z-10">
                              <EmojiPicker 
                                onEmojiClick={(emojiData) => {
                                  addReaction(msg.forum_post_id, emojiData.emoji);
                      ...
                      {replyCount > 0 && (
                        <button
                          onClick={() => toggleThread(msg.forum_post_id)}
                          className="text-sm text-blue-400 hover:text-blue-300 mt-2 flex items-center gap-1"
                        >
                          {expandedThreads.has(msg.forum_post_id) ? '▼' : '▶'} {replyCount} {replyCount === 1 ? 'reply' : 'replies'}
                        </button>
                      )}
                      
                      {/* Thread replies */}
                      {expandedThreads.has(msg.forum_post_id) && replies.length > 0 && (
                        <div className="ml-8 mt-3 space-y-3 border-l-2 border-gray-700 pl-4">
                          {replies.map((reply) => (
                            <div key={reply.forum_post_id} className="flex gap-2 items-start group">
                              <img
                                ...
                                {reply.reactions && Object.keys(reply.reactions).length > 0 && (
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {Object.entries(reply.reactions).map(([emoji, users]) => (
                                      <button
                                        key={emoji}
                                        onClick={() => addReaction(reply.forum_post_id, emoji)}
                                  ...
                                  <div className="relative">
                                    ...
                                    {showReactionPicker === reply.forum_post_id && (
                                      <div className="absolute bottom-full left-0 mb-2 z-10">
                                        <EmojiPicker 
                                          onEmojiClick={(emojiData) => {
                                            addReaction(reply.forum_post_id, emojiData.emoji);
                                            setShowReactionPicker(null);
                                          }}
                                          height={350}
                                          width={300}
          ...
          {selectedFiles.length > 0 && (
            <div className="mb-4 flex flex-wrap gap-2">
              {selectedFiles.map((file, idx) => (
                <div key={idx} className="relative bg-[#3d3848] rounded-lg p-2 flex items-center gap-2">
